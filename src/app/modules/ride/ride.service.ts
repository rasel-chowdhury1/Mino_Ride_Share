...
const createRide = async (payload: IRide) => {
  ...
  try {
    if (isManagerReady()) {
      ...
      // FCM: notify the same nearby drivers via push notification
      ...
      // DB notification: save for each nearby driver
      saveNotificationToDriversByProfileId(notifiedDriverIds, {
        senderId:    ride.passenger.toString(),
        senderName:  passenger?.name ?? '',
        senderImage: passenger?.profileImage ?? '',
        text:        `New ride request from ${passenger?.name ?? 'a passenger'}. Pickup: ${ride.pickupLocation.address}`,
        type:        'newRideRequest',
      });
    }
  } catch (err) {
...
const driverAcceptRide = async (
  ...
  try {
    if (isManagerReady()) {
      ...
      // FCM: notify passenger that driver accepted
      sendNotificationByFcmToken(
        saved.passenger.toString(),
        `${userDoc?.name ?? 'Your driver'} accepted your ride. They are on the way!`,
        'Ride Accepted',
      ).catch((err) => logger.warn('driverAcceptRide: FCM to passenger failed:', err));

      // DB notification: save for passenger
      saveNotification({
        senderId:    driverId,
        receiverId:  saved.passenger.toString(),
        senderName:  userDoc?.name ?? '',
        senderImage: userDoc?.profileImage ?? '',
        text:        `${userDoc?.name ?? 'Your driver'} accepted your ride. They are on the way!`,
        type:        'rideAccepted',
      });
    }
  } catch (err) {
...
const updateRideStatus = async (
  ...
  try {
    if (isManagerReady()) {
      ...
      const statusMessages: Partial<Record<IRide['status'], string>> = {
      ...
      const statusNotifType: Partial<Record<IRide['status'], { type: Parameters<typeof saveNotification>[0]['type']; text: string }>> = {
      ...
      if (fcmBody) {
        sendNotificationByFcmToken(saved.passenger.toString(), fcmBody, 'Mino Ride')
          .catch((err) => logger.warn('updateRideStatus: FCM to passenger failed:', err));
      }

      // DB notification: save for passenger
      const notifEntry = statusNotifType[status];
      if (notifEntry && saved.driver) {
        saveNotification({
          senderId:   saved.driver.toString(),
          receiverId: saved.passenger.toString(),
...
const getMyRides = async (
  ...
  const filter    = isDriver
    ? { driver: id,    isDeleted: false }
    : { passenger: id, isDeleted: false };

  const baseQuery = isDriver
    ? Ride.find(filter)
        .populate('passenger', 'name profileImage phoneNumber averageRating totalReview')
        .sort({ createdAt: -1 })
    : Ride.find(filter)
        .populate({
          path:     'driver',
          select:   'userId vehicleBrand vehicleModel vehicleType licenseNumber  totalTrips',
          populate: { path: 'userId', select: 'name profileImage phoneNumber  countryCode averageRating totalReview' },
        })
        .sort({ createdAt: -1 });

  const rideQuery = new QueryBuilder(baseQuery, query)
    .filter()
    .paginate();
...
const endRide = async (
  ...
  try {
    if (isManagerReady()) {
      ...
      const endStatusPayload = { rideId: saved._id.toString(), status: 'END_RIDE', changedAt: new Date(), serviceType: saved.serviceType,
        pickupType:  saved.pickupType, };
...
const arrivedDropoff = async (
  ...
  try {
    if (isManagerReady()) {
      ...
      const statusPayload = { rideId: saved._id.toString(), status: 'ARRIVED_DROPOFF', changedAt: new Date(), serviceType: saved.serviceType,
        pickupType:  saved.pickupType, };
...
const confirmDropoff = async (rideId: string, driverId: string): Promise<IRide> => {
  ...
  try {
    if (isManagerReady()) {
      ...
      const confirmStatusPayload = { rideId: saved._id.toString(), status: 'CONFIRM_DROPOFF', changedAt: new Date(), serviceType: saved.serviceType,
        pickupType:  saved.pickupType, };
...
const payRide = async (rideId: string, passengerId: string, tip = 0): Promise<IRide> => {
  ...
  // CASH: driver physically holds totalFare; deduct adminCommission from wallet
  if (saved.driver) {
    const earning    = saved.driverEarning   ?? 0;
    const commission = saved.adminCommission ?? 0;

    Driver.findByIdAndUpdate(saved.driver, {
      $inc: { walletBalance: -commission, totalEarnings: earning, totalTrips: 1 },
    }).catch((err) => logger.warn('payRide: driver wallet debit failed:', err));

    Driver.findById(saved.driver).select('userId').lean().then((driverDoc) => {
      if (driverDoc && commission > 0) {
        recordWalletTransaction({
          userId:      driverDoc.userId.toString(),
          type:        'DEBIT',
          source:      'ADMIN_COMMISSION',
          amount:      commission,
          description: `Platform commission for ride #${saved.rideId ?? saved._id}`,
          rideId:      saved._id.toString(),
        }).catch((err) => logger.warn('payRide: commission wallet tx record failed:', err));
      }
...
/**
 * Driver confirms they have collected cash from the passenger.
 * Completes the ride and credits the driver's wallet.
 */
const collectCashPayment = async (rideId: string, driverId: string, tip = 0): Promise<IRide> => {
  const ride = await Ride.findById(rideId);
  if (!ride) throw new AppError(httpStatus.NOT_FOUND, 'Ride not found');
  ...
  if (ride.paymentMethod !== 'CASH') {
    throw new AppError(httpStatus.BAD_REQUEST, 'This endpoint is only for CASH rides');
  }

  if (ride.status !== 'CONFIRM_DROPOFF') {
    throw new AppError(httpStatus.BAD_REQUEST, `Cannot collect payment in status: ${ride.status}`);
  }

  if (ride.paymentStatus === 'PAID') {
    throw new AppError(httpStatus.CONFLICT, 'Payment already collected');
  }
  ...
  try {
    await Driver.findByIdAndUpdate(saved.driver, {
      $inc: { walletBalance: -commission, totalEarnings: earning, totalTrips: 1 },
    });

    const driverDoc = await Driver.findById(saved.driver).select('userId').lean();
    if (driverDoc && commission > 0) {
      recordWalletTransaction({
        userId:      driverDoc.userId.toString(),
        type:        'DEBIT',
        ...
        description: `Platform commission for ride #${saved.rideId ?? saved._id}`,
        rideId:      saved._id.toString(),
      }).catch((err) => logger.warn('collectCashPayment: commission wallet tx record failed:', err));
    }
  } catch (err) {
  ...
  try {
    await PaymentService.createPayment({
      ...
      driverEarning,
      adminCommission,
      promo:           saved.promo ?? undefined,
      promoDiscount:   saved.promoDiscount ?? 0,
      tip:             tipAmount,
      paymentMethod:   'CASH',
    });
  } catch (err) {
    logger.warn('collectCashPayment3: payment record creation failed (non-critical):', err);
  }
  ...
  try {
    ...
    logger.warn('collectCashPayment4: socket emission failed (non-critical):', err);
  }
...
const submitRideReview = async (
  ...
  const ride = await Ride.findById(rideId);
  if (!ride) throw new AppError(httpStatus.NOT_FOUND, 'Ride not found');
  if (ride.status !== 'COMPLETED' && ride.status !== 'CANCELLED' && ride.status !== 'CONFIRM_DROPOFF') {
    throw new AppError(httpStatus.BAD_REQUEST, 'Only completed, comfirm_dropoff or cancelled rides can be reviewed');
  }
...
const ACTIVE_STATUSES = ['ONGOING'];

const getMyActiveRide = async (id: string, role: 'passenger' | 'driver') => {
  const filter = role === 'driver'
    ? { driver: id,    status: { $in: ACTIVE_STATUSES }, isDeleted: false }
    : { passenger: id, status: { $in: ACTIVE_STATUSES }, isDeleted: false };

  const ride = role === 'driver'
    ? await Ride.findOne(filter)
        .populate('passenger', 'name profileImage phoneNumber averageRating totalReview')
        .sort({ createdAt: -1 })
    : await Ride.findOne(filter)
        .populate({
          path:     'driver',
          select:   'userId vehicleBrand vehicleModel vehicleType licenseNumber  totalTrips currentLocation',
          populate: { path: 'userId', select: 'name profileImage phoneNumber  countryCode averageRating totalReview' },
        })
        .sort({ createdAt: -1 });
...
const getRideReview = async (rideId: string, requesterId: string, role: 'passenger' | 'driver') => {
  const ride = await Ride.findById(rideId)
    .select('passenger driver passengerReview driverReview isPassengerReviewed isDriverReviewed status')
    .lean();
  ...
  if (role === 'passenger') {
    if (ride.passenger.toString() !== requesterId) {
      throw new AppError(httpStatus.FORBIDDEN, 'You are not the passenger of this ride');
    }
    return {
      myReview:       ride.passengerReview    ?? null,
      theirReview:    ride.driverReview       ?? null,
      isReviewed:     ride.isPassengerReviewed,
      isTheirReviewed: ride.isDriverReviewed,
    };
  }

  // Driver — look up by driverProfileId
  const driverDoc = await Driver.findOne({ _id: requesterId }).lean();
  if (!driverDoc || ride.driver?.toString() !== requesterId) {
    throw new AppError(httpStatus.FORBIDDEN, 'You are not the driver of this ride');
  }
  return {
    myReview:        ride.driverReview        ?? null,
    theirReview:     ride.passengerReview     ?? null,
