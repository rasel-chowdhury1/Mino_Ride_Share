...
const createCheckoutSession = async (
  ...
  const session = await stripeClient.checkout.sessions.create({
    ...
    success_url: `${config.stripe.stripe_success_url}?rideId=${rideId}&session_id={CHECKOUT_SESSION_ID}`,
    cancel_url:  `${config.stripe.stripe_cancel_url}?rideId=${rideId}`,
  });
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    try {
      if (isManagerReady()) {
        ...
        emitToRideRoom(rideId, SocketEvents.RIDE_STATUS_UPDATED, { rideId, status: 'COMPLETED', changedAt: new Date() });

        if (ride.driver) {
          emitToDriver(ride.driver.toString(), SocketEvents.RIDE_COMPLETED, payload);
